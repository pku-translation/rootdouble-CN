...
public static class LZSS
{
    ...
    public static IEnumerable<byte> Encode(IEnumerable<byte> bytes)
    {
        ...
        //InitTree();
        for (var i = N + 1; i <= N + 256; ++i) rson[i] = Nil;
        for (var i = 0; i < N; ++i) dad[i] = Nil;

        void InsertNode(int _r)
        {
            var cmp = 1;
            var key_index = _r;
            var p = N + 1 + text_buf[key_index];
        ...
        for (var i = 1; i <= F; ++i) InsertNode(r - i);
        InsertNode(r);
        do {
            int i;
            ...
            var last_match_length = match_length;
        ...
        if (code_buf_ptr > 1) {
            for (var i = 0; i < code_buf_ptr; ++i) yield return code_buf[i];
            codesize += code_buf_ptr;
        }
    }
